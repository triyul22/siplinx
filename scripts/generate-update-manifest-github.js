 ...
 *
 * Usage:
 *   node scripts/generate-update-manifest-github.js <version> [bundle-dir] [output-file] [notes] [repository]
 *
 * Example:
 *   node scripts/generate-update-manifest-github.js 0.3.26 frontend/src-tauri/target/release/bundle/updater latest.json "Release notes here" aman-tiger/siplinxai
 */
...
const [
  ...
  notes = '',
  repository = process.env.GITHUB_REPOSITORY || 'aman-tiger/siplinxai',
] = process.argv.slice(2);

if (!version) {
  console.error('Usage: node generate-update-manifest-github.js <version> [bundle-dir] [output-file] [notes] [repository]');
  console.error('Example: node generate-update-manifest-github.js 0.3.26 frontend/src-tauri/target/release/bundle/updater latest.json "Release notes" aman-tiger/siplinxai');
  process.exit(1);
}
...
bundleFiles.forEach(filename => {
  ...
  if (platform && !platforms[platform]) {
    ...
    const githubUrl = `https://github.com/${repository}/releases/download/${versionDir}/${encodeURIComponent(filename)}`;
