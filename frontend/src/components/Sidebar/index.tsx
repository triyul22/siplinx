...
const Sidebar: React.FC = () => {
  ...
  const handleSaveModelConfig = async (config: ModelConfig) => {
    try {
      const normalized = normalizeCloudOnlySummaryConfig(config);
      ...
      await emit('model-config-updated', normalized);

      // Track settings change
      await Analytics.trackSettingsChanged('model_config', `${normalized.provider}_${normalized.model}`);
    } catch (error) {
  ...
  const handleDelete = async (itemId: string) => {
    ...
    try {
      ...
      toast.success(t("sidebar.deleteSuccessTitle"), {
        description: t("sidebar.deleteSuccessDesc")
      });
  ...
  const handleEditConfirm = async () => {
    ...
    if (!newTitle) {
      toast.error(t("sidebar.editEmptyError"));
      return;
    }
    ...
    try {
      ...
      toast.success(t("sidebar.editSuccess"));
      ...
      toast.error(t("sidebar.editErrorTitle"), {
        description: error instanceof Error ? error.message : String(error)
      });
    }
  };
            ...
            <div className="flex flex-col w-full">
                ...
                {isMeetingItem && (
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                    <button
                      ...
                      aria-label={t("sidebar.editMeetingTitleAria")}
                    >
                    ...
                    <button
                      ...
                      aria-label={t("sidebar.deleteMeetingAria")}
                    >
              ...
              {hasTranscriptMatch && (
                <div className="mt-1 ml-8 text-xs text-gray-500 bg-yellow-50 p-1.5 rounded border border-yellow-100 line-clamp-2">
                  <span className="font-medium text-yellow-600">{t("sidebar.matchLabel")}</span> {matchingResult.matchContext}
                </div>
              )}
            </div>
          )}
    ...
    <div className="fixed top-0 left-0 h-screen z-40">
      ...
      >
        ...
        {!isCollapsed && (
          ...
          <div className="flex-shrink-0 p-2 border-t border-gray-100">
            {/* Индикация триала: «Триал · осталось N дн.», клик = модал оплаты */}
            <TrialBadge />
            {betaFeatures.importAndRetranscribe && (
              <button
                onClick={() => openImportDialog()}
      ...
      <Dialog open={editModalState.isOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-[425px]">
          <VisuallyHidden>
            <DialogTitle>{t("sidebar.editModalTitle")}</DialogTitle>
          </VisuallyHidden>
          <div className="py-4">
            <h3 className="text-lg font-semibold mb-4">{t("sidebar.editModalTitle")}</h3>
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder={t("sidebar.meetingTitlePlaceholder")}
                  autoFocus
                />
              </div>
            </div>
