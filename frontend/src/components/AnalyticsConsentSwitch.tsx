...
export default function AnalyticsConsentSwitch({
  ...
  /** "compact" — только тумблер (для строки в настройках), "full" — блок с пояснениями и User ID. */
      ...
      <>
        <div className="flex items-center gap-2">
          ...
          <Switch
            ...
            aria-label={t("dialogs.consent.enableTitle")}
  ...
  return (
    <>
      <div className="space-y-4">
        <div>
          <h3 className="text-base font-semibold text-gray-800 mb-2">{t("dialogs.consent.heading")}</h3>
          <p className="text-sm text-gray-600 mb-4">
            {t("dialogs.consent.intro")}
          </p>
        </div>

        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
          <div>
            <h4 className="font-semibold text-gray-800">{t("dialogs.consent.enableTitle")}</h4>
            <p className="text-sm text-gray-600">
              {isProcessing ? t("dialogs.consent.updating") : t("dialogs.consent.anonOnly")}
            </p>
          </div>
          ...
          <div className="p-4 border rounded-lg bg-gray-50">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="font-medium text-gray-800 mb-1">{t("dialogs.consent.yourUserId")}</div>
                <p className="text-xs text-gray-600 mb-2">
                  {t("dialogs.consent.userIdHint")}
                </p>
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    title={t("dialogs.consent.copyUserId")}
                  >
                    {isCopied ? (
                      <>
                        <Check className="w-3.5 h-3.5 text-green-600" />
                        <span className="text-green-600">{t("dialogs.consent.copied")}</span>
                      </>
                    ) : (
                      <>
                        <Copy className="w-3.5 h-3.5" />
                        <span>{t("dialogs.consent.copy")}</span>
                      </>
                    )}
                  </Button>
                </div>
