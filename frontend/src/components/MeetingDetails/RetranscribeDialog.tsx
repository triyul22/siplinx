...
export function RetranscribeDialog({
      ...
      const unlistenComplete = await listen<RetranscriptionResult>(
        ...
        async (event) => {
          if (event.payload.meeting_id === meetingId) {
            ...
            setIsProcessing(false);
            toast.success(
              t("dialogs.retranscribe.toastComplete", { count: event.payload.segments_count })
            );
  ...
  const handleStartRetranscription = async () => {
    if (!meetingFolderPath) {
      setError(t("dialogs.retranscribe.errNoFolderPath"));
      return;
    }
  ...
  const handleCancel = async () => {
    if (isProcessing) {
      try {
        ...
        setIsProcessing(false);
        setProgress(null);
        toast.info(t("dialogs.retranscribe.toastCancelled"));
      } catch (err) {
          ...
          <DialogTitle className="flex items-center gap-2">
            {isProcessing ? (
              <>
                ...
                {t("dialogs.retranscribe.titleProcessing")}
              </>
            ) : error ? (
              <>
                <AlertCircle className="h-5 w-5 text-red-600" />
                {t("dialogs.retranscribe.titleFailed")}
              </>
            ) : (
              <>
                <RefreshCw className="h-5 w-5 text-blue-600" />
                {t("dialogs.retranscribe.title")}
              </>
            )}
          </DialogTitle>
          <DialogDescription>
            {isProcessing
              ? progress?.message || t("dialogs.retranscribe.descProcessing")
              : error
                ? t("dialogs.retranscribe.descError")
                : t("dialogs.retranscribe.desc")}
          </DialogDescription>
        </DialogHeader>
            ...
            !isParakeetModel ? (
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium">{t("dialogs.retranscribe.language")}</span>
                </div>
                <Select value={selectedLang} onValueChange={setSelectedLang}>
                  <SelectTrigger className="w-full">
                    <SelectValue placeholder={t("dialogs.retranscribe.selectLanguage")} />
                  </SelectTrigger>
                ...
                <p className="text-xs text-muted-foreground">
                  {t("dialogs.retranscribe.languageHint")}
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                ...
                <p className="text-xs text-muted-foreground">
                  {t("dialogs.retranscribe.parakeetLanguageNote")}
                </p>
              </div>
            )
          )}
          ...
          {!isProcessing && !error && availableModels.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium">{t("dialogs.retranscribe.model")}</span>
              </div>
              <Select value={selectedModelKey} onValueChange={setSelectedModelKey} disabled={loadingModels}>
                <SelectTrigger className="w-full">
                  <SelectValue placeholder={loadingModels ? t("dialogs.retranscribe.loadingModels") : t("dialogs.retranscribe.selectModel")} />
                </SelectTrigger>
                <SelectContent>
              ...
              <p className="text-xs text-muted-foreground">
                {t("dialogs.retranscribe.modelHint")}
              </p>
            </div>
          )}
