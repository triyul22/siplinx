...
const TranscriptSegment = memo(function TranscriptSegment({
    ...
    return (
        <div
            id={`segment-${id}`}
            className={`mb-3 rounded-lg px-2 py-1 transition-colors ${isFocused ? "bg-[#fbf1de] animate-pulse" : ""}`}
        >
...
export const VirtualizedTranscriptView: React.FC<VirtualizedTranscriptViewProps> = ({
    ...
    const focusedIndex = focusTimestamp == null || segments.length === 0
        ? -1
        : segments.reduce((best, segment, index) =>
            Math.abs(segment.timestamp - focusTimestamp) < Math.abs(segments[best].timestamp - focusTimestamp)
                ? index
                : best, 0);
    ...
    useEffect(() => {
        ...
        document.getElementById(`segment-${segments[focusedIndex].id}`)?.scrollIntoView({
            block: "center",
            behavior: "smooth",
        });
    }, [focusedIndex, focusTimestamp, segments, useVirtualization, virtualizer]);
                            ...
                            return (
                                ...
                                >
                                    <TranscriptSegment
                                        ...
                                        isStreaming={isStreaming}
                                        showConfidence={showConfidence}
                                        isFocused={segment.id === segments[focusedIndex]?.id}
                                    />
                                </motion.div>
                            );
                        })}
