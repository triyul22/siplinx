...
export function DeviceSelection({ selectedDevices, onDeviceChange, disabled = false }: DeviceSelectionProps) {
  ...
  const fetchDevices = async () => {
    try {
      ...
      setError(t('recording.devicesLoadFailed'));
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const startAudioLevelMonitoring = async () => {
    try {
      ...
      if (deviceNames.length === 0) {
        setError(t('recording.noMicToMonitor'));
        return;
      }
      ...
      setError(t('recording.monitoringStartFailed'));
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{t('recording.noMicDevices')}</p>
          )}

          {/* Audio Level Meters for Input Devices */}
          {showLevels && inputDevices.length > 0 && (
            <div className="space-y-2 pt-2 border-t border-gray-100">
              <p className="text-xs text-gray-600 font-medium">{t('recording.micLevels')}</p>
              {inputDevices.map((device) => {
        ...
        <div className="space-y-2">
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{t('recording.noSystemDevices')}</p>
          )}
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• <strong>{t('recording.microphone')}:</strong> {t('recording.infoMic')}</p>
        <p>• <strong>{t('recording.systemAudio')}:</strong> {t('recording.infoSystem')}</p>
        {isMonitoring && (
          <p>• <strong>{t('recording.micLevels')}</strong> {t('recording.infoLevels')}</p>
        )}
        {!isMonitoring && inputDevices.length > 0 && (
          <p>• <strong>{t('recording.tipLabel')}:</strong> {t('recording.tipTestMic')}</p>
        )}
      </div>
    </div>
  );
