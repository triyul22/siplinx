...
export function ModelManager({
  ...
  useEffect(() => {
    ...
    const initializeModels = async () => {
      try {
        ...
        setError(err instanceof Error ? err.message : t('settings.model.loadFailed'));
        toast.error(t('settings.toast.modelsLoadFailed'), {
          description: err instanceof Error ? err.message : t('settings.toast.unknownError'),
          duration: 5000
        });
      } finally {
        setLoading(false);
    ...
    const setupListeners = async () => {
      ...
      unlistenComplete = await listen<{ modelName: string }>(
        ...
        (event) => {
          ...
          toast.success(t('settings.toast.modelReady', { icon: getModelIcon(model?.accuracy || 'Good'), model: displayName }), {
            description: t('settings.toast.modelReady.description'),
            duration: 4000
          });
      ...
      unlistenError = await listen<{ modelName: string; error: string }>(
        ...
        (event) => {
          ...
          toast.error(t('settings.toast.modelDownloadFailed', { model: displayName }), {
            description: error,
            duration: 6000,
            action: {
              label: t('settings.model.retry'),
  ...
  const cancelDownload = async (modelName: string) => {
    ...
    try {
      ...
      toast.info(t('settings.toast.modelDownloadCancelled', { model: displayName }), {
        duration: 3000
      });
    } catch (err) {
      console.error('Failed to cancel download:', err);
      toast.error(t('settings.toast.cancelDownloadFailed'), {
        description: err instanceof Error ? err.message : t('settings.toast.unknownError'),
        duration: 4000
      });
    }
  ...
  const downloadModel = async (modelName: string) => {
    ...
    try {
      ...
      toast.info(t('settings.toast.downloadingModel', { model: displayName }), {
        description: t('settings.toast.downloadingModel.description'),
        duration: 5000
      });
  ...
  const selectModel = async (modelName: string) => {
    ...
    const displayName = getDisplayName(modelName);
    toast.success(t('settings.toast.switchedTo', { model: displayName }), {
      duration: 3000
    });
  };
  ...
  const deleteModel = async (modelName: string) => {
    ...
    try {
      ...
      toast.success(t('settings.toast.modelDeletedSimple', { model: displayName }), {
        description: t('settings.toast.modelDeleted.description'),
        duration: 3000
      });
      ...
      toast.error(t('settings.toast.modelDeleteFailed', { model: displayName }), {
  ...
  if (error) {
    return (
      <div className={`bg-red-50 border border-red-200 rounded-lg p-4 ${className}`}>
        <p className="text-sm text-red-800">{t('settings.model.loadFailed')}</p>
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      {advancedModels.length > 0 && (
        <Accordion type="single" collapsible className="w-full">
          <AccordionItem value="advanced-models">
            <AccordionTrigger>
              <span className='text-lg'>{t('settings.model.advancedModels')}</span>
            </AccordionTrigger>
            <AccordionContent>
      ...
      {selectedModel && (
        ...
        >
          {t('settings.model.usingForTranscription', { model: getDisplayName(selectedModel) })}
        </motion.div>
      )}
    </div>
  );
        ...
        <div className="flex items-start justify-between mb-2">
          <div className="flex-1">
            ...
            <div className="flex items-center space-x-4 text-sm text-gray-600 ml-9 mt-1.5">
              ...
              <span className="flex items-center space-x-1">
                <span>🎯</span>
                <span>{t('settings.model.accuracy', { accuracy: model.accuracy })}</span>
              </span>
              <span className="flex items-center space-x-1">
                <span>⚡</span>
                <span>{t('settings.model.processing', { speed: model.speed })}</span>
              </span>
            </div>
          </div>
            ...
            {isAvailable && (
              <>
                <div className="flex items-center gap-1.5 text-green-600">
                  ...
                  <span className="text-xs font-medium">{t('settings.model.ready')}</span>
                </div>
                <AnimatePresence>
                  {isHovered && (
                    <motion.button
                      ...
                      title={t('settings.model.deleteToFreeSpace')}
                    >
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                title={t('settings.model.cancelDownload')}
              >
