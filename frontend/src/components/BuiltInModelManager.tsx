...
export function BuiltInModelManager({ selectedModel, onModelSelect }: BuiltInModelManagerProps) {
  ...
  const fetchModels = async () => {
    try {
      ...
      toast.error(t('settings.toast.modelsLoadFailedSimple'));
    } finally {
      setIsLoading(false);
      setHasFetched(true);
    }
    ...
    const setupListener = async () => {
      unlisten = await listen('builtin-ai-download-progress', (event: any) => {
        ...
        if (status === 'completed') {
          ...
          // Refresh models list
          fetchModels();
          toast.success(t('settings.toast.modelDownloadedSuccess', { model }));
        }
  ...
  const downloadModel = async (modelName: string) => {
    try {
      ...
      toast.error(t('settings.toast.modelDownloadFailedSimple', { model: modelName }));
  ...
  const cancelDownload = async (modelName: string) => {
    try {
      ...
      toast.info(t('settings.toast.downloadCancelled', { model: modelName }));
      setDownloadingModels((prev) => {
        const newSet = new Set(prev);
        newSet.delete(modelName);
        return newSet;
  ...
  const deleteModel = async (modelName: string) => {
    try {
      ...
      toast.success(t('settings.toast.modelDeletedNamed', { model: modelName }));
      fetchModels();
    } catch (error) {
      console.error('Failed to delete model:', error);
      toast.error(t('settings.toast.modelDeleteFailedNamed', { model: modelName }));
    }
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h4 className="text-sm font-bold">{t('settings.builtin.title')}</h4>
      </div>
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {isCorrupted && (
                      <span className="px-2 py-0.5 text-xs font-medium bg-red-100 text-red-700 rounded flex items-center gap-1">
                        ...
                        {t('settings.builtin.corrupted')}
                      </span>
                    )}
                    {isError && (
                      <span className="px-2 py-0.5 text-xs font-medium bg-red-100 text-red-700 rounded">
                        {t('settings.builtin.error')}
                      </span>
                    )}
                    {isNotDownloaded && !modelIsDownloading && (
                      <span className="text-xs text-gray-600 font-medium">
                        {t('settings.builtin.notDownloaded')}
                      </span>
                    )}
                  </div>
                    ...
                    {(isError || isCorrupted) && (
                      <p className="mb-1 text-xs text-red-600">
                        {isError && typeof model.status === 'object' && 'Error' in model.status
                          ...
                          : isCorrupted
                          ? t('settings.builtin.fileCorrupted')
                          : t('settings.builtin.anErrorOccurred')}
                      </p>
                    )}
                    <div className="text-xs text-gray-500">
                      <span>{t('settings.builtin.sizeTokens', { size: model.size_mb, tokens: model.context_size })}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="ml-4 flex items-center gap-2">
                  ...
                  {isAvailable && !modelIsDownloading && selectedModel !== model.name && (
                    <button
                      ...
                      title={t('settings.builtin.deleteModel')}
                    >
              ...
              {modelIsDownloading && progress !== undefined && (
                <div className="mt-3 pt-3 border-t border-gray-200">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-medium text-gray-900">{t('settings.downloading')}</span>
