  ...
  return (
    <div className="px-4 py-2">
      ...
      {transcripts?.map((transcript, index) => {
        ...
        const displayText = originalWasEmpty && !isStreaming ? t('misc.transcriptView.silence') : filteredText;
        ...
        const sizerText = cleanStopWords(isStreaming ? streamingTranscript.fullText : transcript.text)
          || (originalWasEmpty && !isStreaming ? t('misc.transcriptView.silence') : '');
          ...
          {isRecording ? (
            <>
              ...
              <p className="text-xs mt-1 text-gray-400">
                {isPaused
                  ? t('misc.transcriptView.clickResume')
                  : t('misc.transcriptView.speakToSee')}
              </p>
            </>
          ) : (
            <>
