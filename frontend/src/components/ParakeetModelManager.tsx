    ...
    const setupListeners = async () => {
      ...
      unlistenComplete = await listen<{ modelName: string }>(
        ...
        (event) => {
          ...
          toast.success(t('settings.toast.modelReady', { icon: displayInfo?.icon || '✓', model: displayName }), {
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      {selectedModel && (
        ...
        >
          {t('settings.model.usingForTranscription', { model: getModelDisplayName(selectedModel) })}
        </motion.div>
      )}
    </div>
  );
