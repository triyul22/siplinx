  ...
  const handleStartRecording = useCallback(async () => {
    ...
    try {
      ...
      if (errorMsg.includes('microphone') || errorMsg.includes('mic') || errorMsg.includes('input')) {
        setDeviceError({
          title: t('recording.err.micTitle'),
          message: t('recording.err.micMessage')
        });
      } else if (errorMsg.includes('system audio') || errorMsg.includes('speaker') || errorMsg.includes('output')) {
        setDeviceError({
          title: t('recording.err.systemTitle'),
          message: t('recording.err.systemMessage')
        });
      } else if (errorMsg.includes('permission')) {
        setDeviceError({
          title: t('recording.err.permissionTitle'),
          message: t('recording.err.permissionMessage')
        });
      } else {
        setDeviceError({
          title: t('recording.err.genericTitle'),
          message: t('recording.err.genericMessage')
        });
      }
    }
        ...
        <div className="flex items-center space-x-2 bg-white rounded-full shadow-lg px-4 py-2">
          {isProcessing && !isParentProcessing ? (
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-gray-600">{t('recording.processingRecording')}</span>
            </div>
          ) : (
            <>
              {showPlayback ? (
