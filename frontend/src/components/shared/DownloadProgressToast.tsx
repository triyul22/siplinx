...
// что модели докачиваются в фоне. Снимаем флаг, когда обязательная
// транскрипционная модель готова; summary теперь cloud-only.
...
export function useDownloadProgressToast() {
  ...
  // Listen to legacy Built-in AI download events if a development build triggers them.
  useEffect(() => {
  ...
  // Show "You're all set" once when the required transcription model has finished downloading.
  useEffect(() => {
