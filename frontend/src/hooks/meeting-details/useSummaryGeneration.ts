...
import { CLOUD_SUMMARY_MODEL } from '@/config/summaryDefaults';
...
export function useSummaryGeneration({
  ...
  const processSummary = useCallback(async ({
    ...
    try {
      ...
      // Show toast notification for generation start (no model names in UI)
      toast.info(isRegeneration ? t('home.summaryRegenerating') : t('home.summaryGenerating'), {
        duration: 3000,
      });

      // PRO + cloud summary enabled: always use Siplinx cloud, regardless of
      // stale local provider settings such as Ollama from older installs.
      let effectiveProvider: string = modelConfig.provider;
      let cloudAuthToken: string | null = null;

      const token = await getCloudSummaryToken();
      if (token) {
        effectiveProvider = 'siplinx-cloud';
        cloudAuthToken = token;
      }
      const effectiveModelName = effectiveProvider === 'siplinx-cloud'
        ? CLOUD_SUMMARY_MODEL
        : modelConfig.model;
      ...
      startSummaryPolling(meeting.id, process_id, async (pollingResult) => {
          ...
          if (isRegeneration) {
            try {
              ...
              if (existingSummary?.data) {
                ...
                toast.error(t('home.summaryFailed'), {
        ...
        if (pollingResult.status === 'completed' && pollingResult.data) {
          ...
          if (pollingResult.data.markdown) {
            ...
            if (pollingResult.data.cloud_fallback === true) {
              toast.warning(t('settings.toast.cloudFallback'));
            }

            // Show success toast
            toast.success(t('home.summaryReady'), {
              duration: 4000,
            });
  ...
  const handleGenerateSummary = useCallback(async (customPrompt: string = '') => {
    ...
    const cloudToken = await getCloudSummaryToken();
    ...
    console.log('🚀 Starting summary generation with config:', {
      provider: cloudToken ? 'siplinx-cloud' : modelConfig.provider,
      model: cloudToken ? CLOUD_SUMMARY_MODEL : modelConfig.model,
      template: selectedTemplate
    });

    // Cloud summary is the preferred PRO path and must not require local
    // Ollama/Gemma model readiness.
    if (cloudToken) {
    ...
    if (modelConfig.provider === 'ollama') {
      try {
        ...
        if (!models || models.length === 0) {
          toast.error(
            'No Ollama models found. Please choose a cloud summary provider in Model Settings.',
            { duration: 5000 }
          );
          return;
        }
    ...
    // Check if built-in AI provider has models available. PRO users with cloud
    // summary enabled do not need the local Gemma model for this action.
