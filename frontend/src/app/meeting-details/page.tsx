...
import { DEFAULT_SUMMARY_MODEL_CONFIG } from "@/config/summaryDefaults";
...
function MeetingDetailsContent() {
  ...
  const setupAutoGeneration = useCallback(async () => {
    ...
    try {
      // PRO cloud summary should auto-generate without checking local Ollama/Gemma.
      if (await getCloudSummaryToken()) {
        console.log('☁️ Cloud summary enabled, auto-generation does not require local models');
      ...
      console.log('💾 DB empty, using Siplinx Cloud as initial default');
      await invoke('api_save_model_config', {
        provider: DEFAULT_SUMMARY_MODEL_CONFIG.provider,
        model: DEFAULT_SUMMARY_MODEL_CONFIG.model,
        whisperModel: DEFAULT_SUMMARY_MODEL_CONFIG.whisperModel,
        apiKey: null,
        ollamaEndpoint: null,
      });
