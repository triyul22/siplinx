      ...
      try {
        ...
        unlistenRecordingStopped = await recordingService.onRecordingStopped(async (payload) => {
          try {
            ...
            if (meetingId) {
              await indexedDBService.markMeetingStopped(meetingId, payload.folder_path);
            }
          } catch (error) {
            console.error('Failed to mark meeting as stopped in IndexedDB:', error);
          }
        });
      } catch (error) {
