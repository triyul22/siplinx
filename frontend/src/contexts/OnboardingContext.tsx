...
export function OnboardingProvider({ children }: { children: React.ReactNode }) {
  ...
  const [summaryModelDownloaded, setSummaryModelDownloaded] = useState(true);
  const [summaryModelProgress, setSummaryModelProgress] = useState(100);
  ...
  const [selectedSummaryModel, setSelectedSummaryModel] = useState<string>(CLOUD_SUMMARY_MODEL);
  ...
  const verifyModelStatus = async (savedStatus: OnboardingStatus) => {
    ...
    let summaryModelDownloaded = true;
    ...
    console.log('[OnboardingContext] Summary uses Siplinx Cloud; no local summary model verification needed');
  ...
  // Start background downloads for transcription models only. Summary is cloud-only.
  const startBackgroundDownloads = async (_includeSummaryModel: boolean) => {
    if (!areLocalModelAutoDownloadsEnabled()) {
      console.log('[OnboardingContext] Local model auto-downloads are disabled for this build');
      setIsBackgroundDownloading(false);
      return;
    }

    console.log('[OnboardingContext] Starting transcription background downloads; summary uses Siplinx Cloud');
    ...
    try {
      ...
      // Hybrid transcription: Kazakh needs Whisper, and Russian on macOS benefits from the
      // Metal-backed Whisper model, so for those locales also pull Whisper in the background.
      // ConfigContext auto-switches to it once ready; until then Parakeet keeps recording working.
      // English/EU stay on the faster Parakeet.
      try {
