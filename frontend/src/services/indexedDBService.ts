...
export interface MeetingMetadata {
  ...
  recoveryStatus?: 'recording' | 'stopped' | 'saved';
  stoppedAt?: number;         // Unix timestamp (ms) when user stopped recording normally
  savedAt?: number;           // Unix timestamp (ms) when meeting was saved to SQLite
...
class IndexedDBService {
  ...
  /**
   * Mark meeting as stopped by the user, before the slower SQLite save finishes.
   */
  async markMeetingStopped(meetingId: string, folderPath?: string | null): Promise<void> {
    try {
      if (!this.db) await this.init();
      ...
      return new Promise((resolve, reject) => {
        ...
        getRequest.onsuccess = () => {
          ...
          if (meeting) {
            const now = Date.now();
            meeting.recoveryStatus = 'stopped';
            meeting.stoppedAt = now;
            meeting.lastUpdated = now;
            if (folderPath) {
      ...
      console.warn('Failed to mark meeting as stopped:', error);
    }
  }
