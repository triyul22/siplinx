...
pub struct AnalyticsConfig {
    ...
    pub enabled: bool,
    /// Real app version (from tauri.conf.json via PackageInfo). CARGO_PKG_VERSION is stuck
    /// at 0.3.0 because releases only bump tauri.conf.json.
...
impl Default for AnalyticsConfig {
    fn default() -> Self {
        Self {
            api_key: String::new(),
            // The PostHog project lives in the EU region (eu.posthog.com/project/209375).
...
impl AnalyticsClient {
    pub async fn new(config: AnalyticsConfig) -> Self {
        let client = if config.enabled && !config.api_key.is_empty() {
            // posthog-rs needs the FULL capture endpoint. Passing just the api key used the
            // crate's default endpoint — the US region — while our project lives in the EU
            // region: events were accepted with HTTP 200 and silently dropped, so the
            // dashboard stayed empty with zero errors in the logs.
            let base = config
                .host
                .as_deref()
                .unwrap_or("https://eu.i.posthog.com");
            let endpoint = format!("{}/i/v0/e/", base.trim_end_matches('/'));
                ...
                .build()
            {
                ...
                Err(e) => {
                    log::error!("Failed to build PostHog client options: {}", e);
                    None
                }
            }
        } else {
    ...
    pub async fn track_event(&self, event_name: &str, properties: Option<HashMap<String, String>>) -> Result<(), String> {
        ...
        // Add app version to all events (real version from tauri.conf.json, not the stale Cargo.toml)
