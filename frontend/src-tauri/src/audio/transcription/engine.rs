...
/// Serializes concurrent model validations. Without this, a background warm-up
/// (meeting detected) racing a recording start would both see "no model loaded"
/// and load the multi-hundred-MB model twice. The second caller now waits,
/// finds the model already loaded and returns instantly.
...
pub async fn validate_transcription_model_ready<R: Runtime>(app: &AppHandle<R>) -> Result<(), String> {
    let _load_guard = MODEL_LOAD_LOCK.lock().await;
