                        ...
                        Some(chunk) => {
                            ...
                            .await
                            {
                                Ok((transcript, confidence_opt, is_partial)) => {
                                    ...
                                    // NB: whisper "confidence" is derived from TEXT LENGTH (bytes/100),
                                    // not token probabilities — a short valid reply like "Привет!" scores
                                    // ~0.23. Threshold 0.3 was silently discarding every utterance under
                                    // ~20 Cyrillic characters; 0.15 keeps short replies and still drops
                                    // near-empty noise. Silence rejection is whisper's no_speech_thold job.
                                    let confidence_threshold = match &engine_clone {
                                        TranscriptionEngine::Whisper(_) | TranscriptionEngine::Provider(_) => 0.15,
