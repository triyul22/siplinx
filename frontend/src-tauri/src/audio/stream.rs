...
impl AudioStreamManager {
    ...
    pub async fn start_streams(
        ...
        // Create microphone and system streams in parallel (separate tasks, since
        // stream creation is synchronous WASAPI/CPAL work with no await points):
        // each creation enumerates audio devices on its own, which costs seconds
        // on some machines, and doing it sequentially doubled the recording
        // start delay.
        let mic_task = microphone_device.map(|mic_device| {
            let state = self.state.clone();
            let sender = recording_sender.clone();
            tokio::spawn(async move {
                info!("🎤 Creating microphone stream: {} (always uses CPAL)", mic_device.name);
                AudioStream::create(mic_device.clone(), state, DeviceType::Microphone, sender)
                    .await
                    .map(|stream| (mic_device, stream))
            })
        });

        let sys_task = system_device.map(|sys_device| {
            let state = self.state.clone();
            let sender = recording_sender.clone();
            tokio::spawn(async move {
                info!("🔊 Creating system audio stream: {} (backend: {:?})", sys_device.name, backend);
                AudioStream::create(sys_device.clone(), state, DeviceType::System, sender)
                    .await
                    .map(|stream| (sys_device, stream))
            })
        });

        // System audio failure is non-fatal (as before): recording continues
        // with microphone only.
        if let Some(task) = sys_task {
            match task.await {
                Ok(Ok((sys_device, stream))) => {
                ...
                Err(e) => {
                    warn!("⚠️ System audio stream task panicked: {}", e);
                }
            }
        } else {
        ...
        // Microphone failure is fatal (as before).
        if let Some(task) = mic_task {
            match task.await {
                Ok(Ok((mic_device, stream))) => {
                ...
                Ok(Err(e)) => {
                    ...
                    // Original sequential code never created the system stream
                    // when the mic failed; drop it to release the device.
                    self.system_stream.take();
                    return Err(e);
                }
                Err(e) => {
                    error!("❌ Microphone stream task panicked: {}", e);
                    self.system_stream.take();
                    return Err(anyhow::anyhow!("Microphone stream task failed: {}", e));
                }
            }
        } else {
