...
pub async fn stop_recording<R: Runtime>(
    ...
    // Step 3: Keep the transcription model loaded between recordings.
    // Unloading here forced every subsequent start to reload the model from disk
    // (3-6 seconds of user-visible delay before recording could begin), so the
    // model now stays in memory for instant restarts. Batch jobs (import /
    // retranscription) still free it via unload_engine_after_batch.
    let _ = app.emit(
        "recording-shutdown-progress",
        serde_json::json!({
    ...
    info!("🧠 All transcript chunks processed. Keeping transcription model loaded for fast restart");
