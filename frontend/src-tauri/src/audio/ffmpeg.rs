...
fn bundled_ffmpeg_names() -> Vec<String> {
    let mut names = vec![EXECUTABLE_NAME.to_string()];

    if let Some(triple) = target_triple() {
        #[cfg(windows)]
        names.push(format!("ffmpeg-{}.exe", triple));

        #[cfg(not(windows))]
        names.push(format!("ffmpeg-{}", triple));
    }
...
fn find_bundled_ffmpeg_in(dir: &Path) -> Option<PathBuf> {
    for name in bundled_ffmpeg_names() {
        let candidate = dir.join(name);
        if candidate.exists() && candidate.is_file() {
            debug!("Found bundled ffmpeg: {:?}", candidate);
    ...
    if let Ok(entries) = std::fs::read_dir(dir) {
        for entry in entries.flatten() {
            ...
            if let Some(name) = path.file_name().and_then(|name| name.to_str()) {
                let looks_like_ffmpeg = name == EXECUTABLE_NAME
                    || (name.starts_with("ffmpeg-") && !name.ends_with(".sig"));

                if looks_like_ffmpeg {
                    debug!("Found bundled ffmpeg by fuzzy match: {:?}", path);
                    return Some(path);
                }
            }
        }
...
fn find_ffmpeg_path_internal() -> Option<PathBuf> {
    ...
    if let Ok(exe_path) = std::env::current_exe() {
        if let Some(exe_folder) = exe_path.parent() {
            if let Some(path) = find_bundled_ffmpeg_in(exe_folder) {
                return Some(path);
            }
        }
    }
    ...
    if let Ok(exe_path) = std::env::current_exe() {
        if let Some(exe_folder) = exe_path.parent() {
            ...
            #[cfg(target_os = "macos")]
            {
                ...
                if let Some(path) = find_bundled_ffmpeg_in(&resources_folder) {
                    return Some(path);
                }
