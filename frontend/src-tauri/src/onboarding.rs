...
pub struct ModelStatus {
    ...
    pub summary: String,   // Cloud summary is ready when onboarding completes
}
...
pub async fn complete_onboarding<R: Runtime>(
    ...
    info!("Completing onboarding with Siplinx Cloud summary model");
    ...
    if let Err(e) = SettingsRepository::save_model_config(
        ...
        error!("Failed to save cloud summary model config: {}", e);
        return Err(format!("Failed to save cloud summary model config: {}", e));
    }
    info!("Saved cloud summary model config: provider=siplinx-cloud, model=gpt-5.4-mini");
    ...
    info!("Onboarding completed successfully with cloud summary");
    Ok(())
}
