...
// Global language preference storage. Default to "auto" (detect, DO NOT translate). The old
// "auto-translate" default silently rendered Russian speech as English when the frontend failed
// to sync a language; "auto" transcribes in the spoken language instead.
...
pub fn restore_main_window<R: Runtime>(app: &AppHandle<R>) {
    ...
    if let Err(e) = app.show() {
        log::warn!("Failed to show macOS application: {}", e);
    }
    ...
    if let Some(window) = app.get_webview_window("main") {
        ...
        log::warn!("Could not find main window to restore");
    }
}
...
pub fn run() {
    ...
    tauri::Builder::default()
        .plugin(tauri_plugin_single_instance::init(|app, argv, _cwd| {
            ...
            if let Some(url) = argv.iter().find(|arg| arg.starts_with("siplinx://")) {
                let _ = app.emit("deep-link-opened", url.clone());
            }
        }))
        // Логи в файл (AppData/../logs/siplinx.log) + stdout. Регистрируется ПЕРВЫМ,
        // чтобы поймать ошибки инициализации остальных плагинов.
        .plugin(
            tauri_plugin_log::Builder::new()
                .targets([
                    tauri_plugin_log::Target::new(tauri_plugin_log::TargetKind::Stdout),
                    tauri_plugin_log::Target::new(tauri_plugin_log::TargetKind::LogDir {
                        file_name: Some("siplinx".into()),
                    }),
                ])
        ...
        // Auth: loopback OAuth + открытие ссылок в системном браузере
        ...
        .setup(|_app| {
            ...
            // Autostart is a product default: register it silently and keep the
            // main window hidden when the OS launches us with --autostart.
            // Normal launches start hidden too (see tauri.conf.json), then show
            // here to avoid a window flash during an autostart launch.
            ...
            match autostart.is_enabled() {
                ...
                Err(e) => log::error!("Failed to query autostart state: {}", e),
            }

            let launched_by_os =
                std::env::args_os().any(|arg| arg == std::ffi::OsStr::new("--autostart"));
            if launched_by_os {
                log::info!("Autostart launch detected; keeping main window hidden in tray");
            } else {
                restore_main_window(_app.handle());
            }
        ...
        .run(|_app_handle, event| {
            match event {
                ...
                tauri::RunEvent::Reopen {
                    ...
                    log::info!(
                        "macOS reopen requested from Dock (has_visible_windows={}); restoring main window",
                        has_visible_windows
                    );
                ...
                tauri::RunEvent::WindowEvent {
                    ...
                    log::info!("Main window close requested; hiding instead of destroying");
                    api.prevent_close();
