...
impl WhisperEngine {
        ...
        match model_info.status {
            ModelStatus::Available => {
                ...
                *self.current_model.write().await = Some(model_name.to_string());
                ...
                // Report the ACTUAL use_gpu decision, not just the detected GPU type — on Windows
                // the adaptive config forces CPU, and logging "Vulkan GPU acceleration" here sent
                // the 0.3.20 crash investigation down the wrong path.
                let acceleration_status = if !adaptive_config.use_gpu {
                    "CPU processing (GPU offload disabled by adaptive config)"
                } else {
                    match (&hardware_profile.gpu_type, flash_attn_enabled) {
    ...
    pub async fn transcribe_audio_with_confidence(&self, audio_data: Vec<f32>, language: Option<String>) -> Result<(String, f32, bool)> {
        ...
        // CPU real-time fix: whisper's encoder always processes a full 30-second window
        // (1500 frames), so a 2-second live VAD chunk costs as much as 30 seconds of audio.
        // On GPU that's negligible; on CPU it made live transcription minutes-per-chunk
        // (0.3.20 field report: 4.5 min for a 1.2s chunk on an i7-10510U). Scaling audio_ctx
        // to the actual chunk length (plus margin for context) cuts encoder work proportionally
        // — the standard whisper.cpp streaming technique. GPU path keeps the full window.
        if !adaptive_config.use_gpu {
            let secs = audio_data.len() as f32 / 16000.0;
            let frames = (((secs + 2.0) / 30.0) * 1500.0).ceil() as i32;
            params.set_audio_ctx(frames.clamp(256, 1500));
        }
    ...
    pub async fn transcribe_audio(&self, audio_data: Vec<f32>, language: Option<String>) -> Result<String> {
        ...
        // CPU real-time fix: scale encoder context to chunk length (see confidence path).
        // For long audio (>=28s) this resolves to the full 1500 frames — a no-op.
        if !adaptive_config.use_gpu {
            let secs = audio_data.len() as f32 / 16000.0;
            let frames = (((secs + 2.0) / 30.0) * 1500.0).ceil() as i32;
            params.set_audio_ctx(frames.clamp(256, 1500));
