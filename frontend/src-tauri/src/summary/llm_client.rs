...
pub enum LLMProvider {
    ...
    BuiltInAI,
    CustomOpenAI,
    /// Siplinx PRO cloud summary — OpenAI via our auth-service.
    /// The actual HTTP call happens in summary/cloud.rs (not in generate_summary).
    SiplinxCloud,
}
...
impl LLMProvider {
    ...
    pub fn from_str(s: &str) -> Result<Self, String> {
        match s.to_lowercase().as_str() {
            ...
            "siplinx-cloud" => Ok(Self::SiplinxCloud),
...
pub async fn generate_summary(
    ...
    // SiplinxCloud is handled entirely in service.rs / cloud.rs before reaching here.
    // If it somehow arrives here, return a clear error so the fallback triggers.
    if provider == &LLMProvider::SiplinxCloud {
        return Err("SiplinxCloud must be handled by cloud.rs, not generate_summary".to_string());
    }
    ...
    let (api_url, mut headers) = match provider {
        ...
        LLMProvider::SiplinxCloud => {
            // This case is handled earlier with early returns
            unreachable!("SiplinxCloud is handled before this match statement")
        }
    };
...
fn provider_name(provider: &LLMProvider) -> &str {
    match provider {
        ...
        LLMProvider::SiplinxCloud => "Siplinx Cloud",
    }
}
