    ...
    fn resolve_helper_binary() -> Result<PathBuf> {
        ...
        if let Ok(exe_path) = std::env::current_exe() {
            if let Some(exe_dir) = exe_path.parent() {
                ...
                #[cfg(target_os = "macos")]
                {
                    let universal = exe_dir.join("llama-helper-universal-apple-darwin");
                    if universal.exists() {
                        log::info!(
                            "Found universal match next to executable: {}",
                            universal.display()
                        );
                        return Ok(universal);
                    }
                ...
                #[cfg(target_os = "macos")]
                {
                    ...
                    log::info!(
                        "Searching for llama-helper in macOS Resources: {}",
                        resources_dir.display()
                    );

                    let bundled = resources_dir.join(&binary_name);
                    if bundled.exists() {
                        log::info!(
                            "Found exact match in macOS Resources: {}",
                            bundled.display()
                        );
                        return Ok(bundled);
                    }

                    let universal = resources_dir.join("llama-helper-universal-apple-darwin");
                    if universal.exists() {
                        log::info!(
                            "Found universal match in macOS Resources: {}",
                            universal.display()
                        );
                        return Ok(universal);
                    }
                    ...
                    if let Ok(entries) = std::fs::read_dir(&resources_dir) {
                        for entry in entries.flatten() {
                            ...
                            if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                                if name.starts_with("llama-helper") && !name.ends_with(".d") {
                                    log::info!(
                                        "Found fuzzy match in macOS Resources: {}",
                                        path.display()
                                    );
                                    return Ok(path);
                                }
