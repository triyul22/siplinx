...
impl SummaryProcessesRepository {
    ...
    pub async fn update_meeting_summary(
        ...
        // Back up the previous version before overwriting so any edit (including
        // chat-driven "edit" commands) can be rolled back via result_backup.
        sqlx::query(
            "UPDATE summary_processes SET result_backup = result, result_backup_timestamp = updated_at, result = ?, updated_at = ? WHERE meeting_id = ?",
        )
